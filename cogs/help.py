            ...
            if command is not None:
                ...
                if command in ["jsk", "jishaku"]:
                    ...
                    if cmd is None:
                        return await ctx.send(
                            f"{self.bot.settings['formats']['error']} **Unknown Command:** The `{command}` command "
                            f"does not exist.")
                    else:
                ...
                if not ctx.guild:
                    embed.add_field(name=f"{self.bot.settings['emoji']['home']} Help: Menu",
                                    value=f"Commands in DMs have no prefix, and you can execute them like so: "
                                          f"`help`\nTo get command usage and information do `help <command>`\nClick ["
                                          f"here]({self.bot.settings['inviteURL']}) to join cairo's server.")
                else:
                    embed.add_field(name=f"{self.bot.settings['emoji']['home']} Help: Menu",
                ...
                if self.bot.settings["ownership"]["multiple"]:
                    if ctx.author.id in self.bot.settings["ownership"]["owners"]:
                        embed.add_field(name=f"{self.bot.settings['emoji']['crown']} BotOwner Commands (1)",
                                        value="`jsk`", inline=False)
                else:
